...
import RepoBadge from './.vitepress/theme/repo/RepoBadge.vue'
...
const updateEmbedCode = (repo) => {
  ...
  badgeUrl.searchParams.set('embed', '1')
  embedCode.value = `<iframe src="${badgeUrl.toString()}" width="980" height="360" frameborder="0" scrolling="no"></iframe>`
}
...
onMounted(async () => {
  ...
  const initialRepo = repoFromQuery || repoOptions.value[0] || fallbackRepos[0]
...
<div v-if="!isEmbed" class="repo-generator">
  <div class="repo-generator-header">
    <h2>Project-level Stats Badge</h2>
    <p>Type a repo name (for example: <code>self-llm</code>) and generate an embeddable card.</p>
  </div>
  ...
  <div class="repo-input-row">
    ...
    <button class="repo-generate-btn" @click="generateBadge">Generate</button>
  </div>
