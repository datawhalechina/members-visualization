...
// 对象类型：季度贡献者
const QuarterlyContributorType = new GraphQLObjectType({
  name: 'QuarterlyContributor',
  fields: {
    login: { type: new GraphQLNonNull(GraphQLString) },
    avatar_url: { type: GraphQLString },
    contributions: { type: GraphQLInt },
    repos: { type: new GraphQLList(GraphQLString) },
    classification: { type: GraphQLString }
  }
})

// 对象类型：季度数据
const QuarterlyDataType = new GraphQLObjectType({
  name: 'QuarterlyData',
  fields: {
    ...
    contributors: { type: new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(QuarterlyContributorType))) }
  }
})
...
const QueryType = new GraphQLObjectType({
  ...
  fields: {
    ...
    quarterlyContributors: {
      type: new GraphQLList(new GraphQLNonNull(QuarterlyDataType)),
      args: {
        quarter: { type: GraphQLString }
      },
      resolve: resolvers.Query.quarterlyContributors
    }
  }
})
...
export const typeDefs = `
  ...
  type Query {
    ...
    quarterlyContributors(quarter: String): [QuarterlyData!]
  }
