...
export const resolvers = {
  Query: {
    ...
    async quarterlyContributors(_, { quarter }) {
      const data = await dataSource.loadQuarterlyContributors()
      if (!data) return []
      ...
      if (Array.isArray(data)) {
        ...
        // Handle if data is keyed by quarter
        result = Object.entries(data).map(([key, val]) => ({
          quarter: key,
          ...val
        }))
      }
      ...
      if (quarter) {
        return result.filter(q => q.quarter === quarter)
      }
